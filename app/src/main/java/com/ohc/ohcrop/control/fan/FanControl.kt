...
class FanControl : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ControlDocumentUtils.createFanDocument()
        SetControlSetting()
        ...
        manualcontrolswitch.setOnClickListener {
            ...
            if(manualcontrolswitch.isChecked){
                ...
                imagePhoto.setImageResource(R.drawable.fan)
            }else{
                manualcontrolswitch.setText("OFF")
                imagePhoto.setImageResource(R.drawable.fanoff)
            }
        }
    ...
    private fun spinnerfromfun() {
        ...
        val arraySpinner = ArrayAdapter(this@FanControl, android.R.layout.simple_spinner_dropdown_item, spinneritems)
    ...
    private fun setAutomaticFun() {
        ...
        if (spinnerFromPosition == 0 || spinnerToPosition == 0 || spinnerDurationPosition == 0) {
            ...
            if (spinnerFromPosition <= spinnerToPosition){
                FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("fan").update(
                    mapOf(
    ...
    private fun setautomaticsettings(){
        val ref = FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("fan")
    ...
    private fun manualswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("fan").update("manualmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }

    private fun setmanualswitchfun() {
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("fan").update("manualmode", false)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
        manualcontrolswitch.isChecked = false
    }
    ...
    private fun controlswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("fan").update("controlmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }
}