package com.ohc.ohcrop.control.irrigation
...
class IrrigationControl : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ControlDocumentUtils.createIrrigationDocument()
        SetControlSetting()
        ...
        manualcontrolswitch.setOnClickListener {
            ...
            if(manualcontrolswitch.isChecked){
                ...
                imagePhoto.setImageResource(R.drawable.irrigation)
            }else{
                manualcontrolswitch.setText("OFF")
                imagePhoto.setImageResource(R.drawable.irrigationoff)
            }
        }
        ...
        onBackPressedCallback = object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                ...
                val intent = Intent(this@IrrigationControl, Control::class.java)
    ...
    private fun spinnerfromfun() {
        ...
        val arraySpinner = ArrayAdapter(this@IrrigationControl, android.R.layout.simple_spinner_dropdown_item, spinneritems)
    ...
    private fun setAutomaticFun() {
        ...
        if (spinnerFromPosition == 0 || spinnerToPosition == 0 || spinnerDurationPosition == 0) {
            ...
            if (spinnerFromPosition <= spinnerToPosition){
                FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("irrigation").update(
                    mapOf(
    ...
    private fun setautomaticsettings(){
        val ref = FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("irrigation")
    ...
    private fun manualswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("irrigation").update("manualmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }

    private fun setmanualswitchfun() {
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("irrigation").update("manualmode", false)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
        manualcontrolswitch.isChecked = false
    }
    ...
    private fun controlswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("irrigation").update("controlmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }
}