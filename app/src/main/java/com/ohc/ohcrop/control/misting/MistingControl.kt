package com.ohc.ohcrop.control.misting
...
class MistingControl : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ControlDocumentUtils.createMistingDocumentData()
        ControlDocumentUtils.createMistingDocument()
        SetControlSetting()
        ...
        manualcontrolswitch.setOnClickListener {
            ...
            if(manualcontrolswitch.isChecked){
                imagePhoto.setImageResource(R.drawable.misting)
                manualcontrolswitch.setText("ON")
            }else{
                manualcontrolswitch.setText("OFF")
                imagePhoto.setImageResource(R.drawable.mistingoff)
            }
        }
    ...
    private fun spinnerfromfun() {
        ...
        val arraySpinner = ArrayAdapter(this@MistingControl, android.R.layout.simple_spinner_dropdown_item, spinneritems)
    ...
    private fun setAutomaticFun() {
        ...
        if (spinnerFromPosition == 0 || spinnerToPosition == 0 || spinnerDurationPosition == 0) {
            ...
            if (spinnerFromPosition <= spinnerToPosition){
                FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("misting").update(
                    mapOf(
    ...
    private fun setautomaticsettings(){
        val ref = FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("misting")
    ...
    private fun manualswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("misting").update("manualmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }

    private fun setmanualswitchfun() {
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("misting").update("manualmode", false)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
        manualcontrolswitch.isChecked = false
    }
    ...
    private fun controlswitchfun() {
        ...
        FirebaseUtils.firestore.collection("user").document(userID).collection("control").document("misting").update("controlmode", switchstatus)
            .addOnSuccessListener { Log.d(ContentValues.TAG, "DocumentSnapshot successfully written!") }
            .addOnFailureListener { e -> Log.w(ContentValues.TAG, "Error writing document",e) }
    }
